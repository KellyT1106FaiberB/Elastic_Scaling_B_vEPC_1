...
void run() {
	...
	for (i = 0; i < g_s1_server_threads_count; i++) {
		pgw_s5_clients[i].conn(SGW2,PGWLB,8000);
		sgw_rep1_clients[i].conn(SGW2, g_sgw_rep1_ip, g_sgw_rep_port);
		sgw_rep2_clients[i].conn(SGW2, g_sgw_rep2_ip, g_sgw_rep_port);
	...
	cout << "SGW Replication server started" << endl;
